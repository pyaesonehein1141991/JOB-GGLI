...
public class CustomerDTO extends CommonDTO {
	...
	public CustomerDTO(String initialId, String fatherName, String idNo, double height, double weight,
			String placeOfBirth, Date dateOfBirth, String labourNo, String birthMark, String salary, int closedPolicy,
			int activePolicy, String bankAccountNo, Gender gender, IdType idType, MaritalStatus maritalStatus,
			OfficeAddress officeAddress, PermanentAddress permanentAddress, ResidentAddress residentAddress,
			ContentInfo contentInfo, Name name, StateCode stateCode, TownshipCode townshipCode,
			IdConditionType idConditionType, List<FamilyInfo> familyInfo, Branch branch, Qualification qualification,
			BankBranch bankBranch, Religion religion, Occupation occupation, String fullIdNo, Industry industry,
			Country country, List<CustomerInfoStatus> customerInfoStatusList, PassportType pType) {
		super();
	...
	public CustomerDTO(CustomerDTO customerDTO) {
		super(customerDTO.isExistsEntity(), customerDTO.getVersion(), customerDTO.getId(),
				customerDTO.getCommonCreateAndUpateMarks());
	...
	public String getFullIdNo() {
		if (idType.equals(IdType.NRCNO)) {
			fullIdNo = stateCode.getCodeNo() + "/" + townshipCode.getTownshipcodeno() + "(" + idConditionType + ")" + idNo;

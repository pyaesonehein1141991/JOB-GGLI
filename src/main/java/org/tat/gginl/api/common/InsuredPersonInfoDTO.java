...
import org.tat.gginl.api.domains.InsuranceHistoryRecord;
import org.tat.gginl.api.domains.InsuredPersonAddon;
import org.tat.gginl.api.domains.InsuredPersonAttachment;
...
import org.tat.gginl.api.domains.SurveyQuestionAnswer;
...
public class InsuredPersonInfoDTO {
	...
	public InsuredPersonInfoDTO(PolicyInsuredPerson pi) {
		...
		if (KeyFactorChecker.isGroupLife(pi.getProduct()) || KeyFactorChecker.isPublicLife(pi.getProduct()) || KeyFactorChecker.isShortTermEndowment(pi.getProduct().getId())) {
	...
	public int getPeriodOfMonths() {
		if (KeyFactorChecker.isPublicLife(product) || KeyFactorChecker.isGroupLife(product) || KeyFactorChecker.isShortTermEndowment(product.getId())
				|| KeyFactorChecker.isStudentLife(product.getId())) {

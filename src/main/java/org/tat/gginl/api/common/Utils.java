...
//	}
	...
	public static boolean isLastDayOfMonth(Date date) {
		...
		if (lastDayOfMonth.getDay() == date.getDay()) {
			result = true;
		}
		return result;
	}
	...
	public static <T> boolean isLifeProposal(T lifeProposal) {
		if (lifeProposal instanceof LifeProposal) {
			return true;
		}
		return false;
	}
	...
	public static double divideNoDecimal(double dividend, double divisor) {
		BigDecimal dividendBig = new BigDecimal(dividend);
		BigDecimal divisorBig = new BigDecimal(divisor);
		return dividendBig.divide(divisorBig, 2, RoundingMode.FLOOR).doubleValue();
	}
...
//	}
	...
	public static Date getNextPaymentDate(Date commenmanceDate, int lastPaymentTerm, int periodMonths, int totalPaymentTimes) {
		Date date = null;
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(commenmanceDate);
		// for nextTerm
		if (lastPaymentTerm < totalPaymentTimes)
			lastPaymentTerm = lastPaymentTerm + 1;
		else
			return null;
		for (int i = 1; i <= lastPaymentTerm; i++) {

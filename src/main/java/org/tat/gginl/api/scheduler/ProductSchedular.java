...
public class ProductSchedular {
	...
	//	List<Agent> agentList = agentService.findAll();
		...
		List<Object> columnNameList = productService.findAllColumnName();
		List<Object[]> dataList = productService.findAllNativeObject();
...
//			String tempDir= fileDir.concat("\\ProductInfo").concat(FileService.getDateToString(new Date()));
			...
			FileOutputStream fos = new FileOutputStream("Product.zip");
			...
			File checksumFile = new File("ProductInfoChecksum".concat(".md5"));
			...
			Files.move(Paths.get(checksumFile.getPath()),Paths.get(tempDir.concat("\\ProductInfoChecksum.md5")),StandardCopyOption.REPLACE_EXISTING);
